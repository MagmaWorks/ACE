    ...
    public class Calculations : CalcBase
    {
        ...
        public override void UpdateCalc()
        {
            ...
            if (Lx.Value > 0 && Ly.Value > 0)
            {
                ...
                Console.WriteLine("Second order check OK");
                CapacityCheck = Column.isInsideCapacity();
                Console.WriteLine("Capacity check OK");
        ...
        public bool UpdateSecondOrderCheck()
        {
            ...
            if (!secondorderx)
            {
                ...
                if(Mxd != null) Mxd.Value = Math.Round(Medx, 1);
            ...
            if (!secondordery)
            {
                ...
                if (Myd != null) Myd.Value = Math.Round(Medy, 1);
        ...
        public (bool, Formula) CheckFireZoneMethod(bool newdesign = false, bool calcOnly = false)
        {
            ...
            // Expressions
            bool res = (NRfi > col.FireLoad.P && Mx > col.FireLoad.Mxd && My > col.FireLoad.Myd);
            ...
            if (!calcOnly)
            {
                ...
                f.Expression.Add(@"M_{xd,fi} = " + Math.Round(col.FireLoad.Mxd) + "kN.m");
                f.Expression.Add(@"M_{yR,fi} = " + Math.Round(My) + " kN.m");
                f.Expression.Add(@"M_{yd,fi} = " + Math.Round(col.FireLoad.Myd) + "kN.m");
        ...
        public void UpdateFireID(bool newdesign = false)
        {
            ...
            else if (c.Shape == GeoShape.LShaped)
            {
                ...
                // Creation of the rebars
                List<Point> rebars = c.GetLShapedRebars();
                for (int i = 0; i < rebars.Count; i++)
                {
            ...
            else if (c.Shape == GeoShape.TShaped)
            {
                ...
                // Creation of the rebars
                List<Point> rebars = c.GetTShapedRebars();
                for (int i = 0; i < rebars.Count; i++)
                {
                    double temp = c.GetRebarTemp(new MWPoint2D(rebars[i].X, rebars[i].Y));
                    SteelData sd = steelData.First(s => s.Temp == temp);
        ...
        public override List<MW3DModel> Get3DModels()
        {
            ...
            double size = 100;
            loadStateMesh.addNode(c.SelectedLoad.Mxd - size, c.SelectedLoad.Myd - size, -c.SelectedLoad.P, new MWPoint2D(0.5, 0.5));
            loadStateMesh.addNode(c.SelectedLoad.Mxd - size, c.SelectedLoad.Myd + size, -c.SelectedLoad.P, new MWPoint2D(0.5, 0.5));
            loadStateMesh.addNode(c.SelectedLoad.Mxd + size, c.SelectedLoad.Myd + size, -c.SelectedLoad.P, new MWPoint2D(0.5, 0.5));
            loadStateMesh.addNode(c.SelectedLoad.Mxd + size, c.SelectedLoad.Myd - size, -c.SelectedLoad.P, new MWPoint2D(0.5, 0.5));
            loadStateMesh.addNode(c.SelectedLoad.Mxd, c.SelectedLoad.Myd, -c.SelectedLoad.P - size, new MWPoint2D(0.5, 0.5));
            loadStateMesh.addNode(c.SelectedLoad.Mxd, c.SelectedLoad.Myd, -c.SelectedLoad.P + size, new MWPoint2D(0.5, 0.5));
